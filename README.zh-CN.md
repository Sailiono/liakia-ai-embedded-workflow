...
还在为反复编译、烧录、串口验证和交接整理而消耗精力吗？

Liakia 把这些容易断开的手工步骤整理成一条可复现、可审查、可交接的人审闭环。
...
如果要跑包含持久化写入或 reset 的完整硬件回归，需要显式打开危险 shell 命令：
...
`tools/functional_test.ps1` 和 `tools/run_test_baseline.ps1` 是 dpiny-RTK 参考工程 runner。接入新 STM32 项目时，应从 `workflow-template/project-adapter.json` 和 `workflow-template/run_workflow.ps1` 开始。
