...
function Calc-CRC24Q([byte[]]$data) {
    $crc = 0x000000
    foreach ($b in $data) {
        $idx = (($crc -shr 16) -bxor $b) -band 0xFF
        $crc = (($crc -shl 8) -band 0xFFFFFF) -bxor $crc24q_table[$idx]
    }
    return $crc
}

# RTCM message type names
$msgNames = @{
    1005 = "Station XYZ (ECEF)"
    1074 = "GPS MSM4 Full PR&PR&CNR"
    1084 = "GLONASS MSM4"
    1094 = "Galileo MSM4"
    1124 = "BDS MSM4 Full PR&PR&CNR"
    1019 = "GPS Ephemeris"
...
while ($idx -lt $raw.Count - 5) {
    ...
    if ($idx + 2 -ge $raw.Count) { break }

    # Parse header
    $hdr = ($raw[$idx+1] -shl 8) -bor $raw[$idx+2]
    $msgLen = $hdr -band 0x3FF  # 10 bits
    $frameLen = 1 + 2 + $msgLen + 3  # preamble + header + content + CRC

    if ($idx + $frameLen -gt $raw.Count) { break }

    # Extract frame
    $frame = $raw[$idx..($idx + $frameLen - 1)]

    # Verify CRC
    $calcCrc = Calc-CRC24Q $frame[0..($frameLen - 4)]
    $rcvCrc = ($frame[$frameLen-3] -shl 16) -bor ($frame[$frameLen-2] -shl 8) -bor $frame[$frameLen-1]

    # Parse message number (first 12 bits of content)
    $msgNum = (($frame[3] -shl 4) -bor ($frame[4] -shr 4)) -band 0xFFF

    $crcOk = ($calcCrc -eq $rcvCrc)
    $name = if ($msgNames.ContainsKey($msgNum)) { $msgNames[$msgNum] } else { "Unknown" }
    ...
    if (-not $msgCount.ContainsKey($msgNum)) { $msgCount[$msgNum] = 0 }
    $msgCount[$msgNum]++
...
# Results
Write-Host "=== RTCM Stream Analysis ===" -ForegroundColor Cyan
Write-Host "Total frames found: $($frames.Count)"
Write-Host ""

Write-Host "--- Message Types ---"
foreach ($k in ($msgCount.Keys | Sort-Object)) {
    $name = if ($msgNames.ContainsKey($k)) { $msgNames[$k] } else { "Unknown" }
    Write-Host ("  MT{0}: {1} ({2} msgs)" -f $k, $name, $msgCount[$k])
}

Write-Host ""
Write-Host "--- CRC Status ---"
$ok = ($frames | Where-Object { $_.CRC_OK }).Count
$bad = ($frames | Where-Object { -not $_.CRC_OK }).Count
Write-Host "  CRC OK: $ok" -ForegroundColor Green
Write-Host "  CRC BAD: $bad" -ForegroundColor $(if ($bad -gt 0) { "Red" } else { "Green" })

Write-Host ""
Write-Host "--- Sample Frames ---"
$frames | Select-Object -First 15 | Format-Table Index, MsgNum, Name, Length, CRC_OK -AutoSize
