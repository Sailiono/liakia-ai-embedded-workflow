# Adapt Your STM32 Project To BenchLoop

BenchLoop is not tied to the dpiny-RTK firmware. The goal is to wrap an existing STM32 project with a repeatable delivery loop:
...
BenchLoop uses an adapter-style description so the workflow can stay generic while project details remain explicit.
...
BenchLoop should keep the final engineering decision with humans.
