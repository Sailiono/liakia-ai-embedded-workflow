...
主仓库里的 dpiny-RTK 工程负责证明：Liakia 可以在真实 STM32F407 + RTK 固件上跑通编译、烧录、测试、诊断、证据归档和交接流程。Starter-F103 Lab 的作用不同。它不是再展示一个已经跑好的项目，而是让你自己搭一套低成本 F103 实验台架，自己生成 IOC，自己接入应用层，再导入我们故意写错的故障代码，完整体验一次“失败 -> 证据 -> AI 诊断 -> 最小修复 -> 回归 PASS”。
...
[known-bad-cases/case-a-bmp280-calibration/README.zh-CN.md](known-bad-cases/case-a-bmp280-calibration/README.zh-CN.md)
...
| Case | 需要导入什么 | 最适合验证什么 |
|---|---|---|
| [Case A：BMP280 数据质量失败](known-bad-cases/case-a-bmp280-calibration/README.zh-CN.md) | 完整替换 `liakia_lab_app.c` | 第一次体验证据驱动诊断 |
| [Case B：I2C reset 后恢复失败](known-bad-cases/case-b-i2c-bus-stuck-reset/README.zh-CN.md) | 替换 port 层文件 | reset 后状态推理 |
| [Case C：Flash 配置持久化失败](known-bad-cases/case-c-flash-persistence-alignment/README.zh-CN.md) | 配置持久化片段 | reset 后状态和原始记录证据 |
| [Case D：UART DMA/IDLE 数据流失败](known-bad-cases/case-d-uart-dma-idle-race/README.zh-CN.md) | DMA/IDLE 接收片段 | 高阶串口诊断 |
